...
class VAE(nn.Module):
	def __init__(self):
		...
		conv_e = nn.Sequential(
				...
				nn.AvgPool2d(kernel_size=2, stride=2),
				nn.Conv2d(64, 256, kernel_size=5, bias=True, stride=2),
				nn.ELU(),
		...
		#Takes z latent variable of size 100
				...
				nn.Conv2d(256, 64, kernel_size=5, bias=True, stride=1, padding=2),
				nn.ELU(),
				...
				nn.ELU(),
				nn.Conv2d(16, 3, kernel_size=3, bias=True, stride=1, padding=2)
...
if __name__ == "__main__":
	...
	#				   'results/sample_' + str(epoch) + '.png', normalize=True, scale_each=True)
			sample = torch.randn(16, 100).to(device)
			sample = model.decode(sample)
			save_image(sample.view(16, 3, 32, 32),
