...
def disentangled(z, model, epsilon=3):
    ...
    for i in range(size):
      if i==0:
        ...
        if model=='VAE':
          sample = model.decode(new_z).view(1, 3, 32, 32)
        else:
          sample0 = model(new_z) 
          sample = torch.cat([sample,sample0])
        for i in range(size-1):
    ...
    print(sample.shape)
    save_image(sample.data[0:25],data_path+'Disentangled/Allsample_' + str(dim1)+'-'+str(dim2) + '.png', nrow=5, normalize=True)
    #new_z = z.clone()
