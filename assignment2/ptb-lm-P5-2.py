...
from models5 import RNN, GRU
from models5 import make_model as TRANSFORMER
...
def run_epoch(model, data, is_train=False, lr=1.0):
    ...
    # here we store the loss at each time step for each minibatch
    loss_t_hist = torch.zeros(epoch_size,model.seq_len)
    # here we compute the average loss of the whole epoch, for debuging purposes
    avg_loss = 0.0
    ...
    for step, (x, y) in enumerate(ptb_iterator(data, model.batch_size, model.seq_len)):
                ...
                for t in range(seq_len):
                    ...
                    if t == seq_len-1:
                        ...
                        for i in range(seq_len):
                            avg_grad_ht_LT.append(torch.mean(g[i],dim=1))
                            grads_norm[i] = torch.norm(avg_grad_ht_LT[i],p='fro').cpu().numpy()

                        np.save(args.save_dir + '/gradsL_wrt_ht',grads_norm)
        ...
        if is_train == False:
            for t in range(model.seq_len):
                outputs_t = outputs[t,:,:].data
                targets_t = targets[t,:].data
                loss_t = loss_fn(outputs_t,targets_t)
                loss_t_hist[step,t] = loss_t

        # print(torch.mean(loss_t_hist[step,:]).item())
    ...
    if args.debug:
        print('epoch avg loss computed from the loss history: ' + str(torch.mean(loss_t_hist).item()))
        print('epoch avg loss computed in LOSS COMPUTATION: ' + str(avg_loss))
